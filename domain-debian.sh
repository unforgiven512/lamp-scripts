...
mkdir -p $DOMAIN_PATH/public_html $DOMAIN_PATH/{logs,awstats,awstats/.data}
...
sed -i 's/^LogFile=.*/LogFile="\/srv\/www\/'${DOMAIN}'\/logs\/access.log"/' /etc/awstats/awstats.$DOMAIN.conf
sed -i 's/^LogFormat=.*/LogFormat=1/' /etc/awstats/awstats.$DOMAIN.conf
sed -i 's/^DirData=.*/DirData="\/srv\/www\/'${DOMAIN}'\/awstats\/\.data"/' /etc/awstats/awstats.$DOMAIN.conf
...
if [ ! -n "$1" ]; then
    ...
    echo -n  "$0"
    echo -ne "\033[36m add Domain.tld\033[0m"
    echo     " - Add specified domain. Awstats and log rotation will be configured."

    echo -n  "$0"
    echo -ne "\033[36m remove Domain.tld\033[0m"
...
remove)
	DOMAIN=$2
    initialize_variables
#Force user to enter a domain as argument
	if [ $# -ne 2 ]; then
	    echo "Please enter all the required parameters."
		exit
	fi
	...
	if [ $? = 0  ]; then
		...
		echo -e "\033[35;1m$DOMAIN_CONFIG_PATH does not exist. \033[0m"
		exit
	fi
  ;;
awstats)
